...
fn main() -> std::io::Result<()> {
    ...
    let mut f = std::io::BufReader::new(std::fs::File::open("test.txt")?);
    //let mut b : Vec<u8> = Vec::with_capacity(4);
    let y = 7;
    let mut b :Vec<u8> = vec![0; y];
    println!("{}", b[0]);
    f.seek(SeekFrom::Start(2))?;
    let sz = f.read(&mut b[0..4])?;
    for i in 0..sz{
        println!("byte {}",char::from(b[i]));
    }
    let sz = f.read(&mut b[0..4])?;
    for i in 0..sz{
        println!("byte {}",char::from(b[i]));
    ...
    a.push(4);
    println!("a = {} {}", a.len(), a[1]);
    let ar = vec![1,2,3,4,5,5,5,5];
    let mut b = &ar[..];
    b = &b[1..];
    println!("the ritme {} {}", a.len(), b.len());

    //let mut vec : Vec<Vec<u8>> = Vec::with_capacity(5);
    //vec.push(vec![1,2]);
    //vec[0].copy_from_slice(&[1,2,3]);
...
fn byte_trim_end<'a>( bytes : &'a [u8], cut : &[u8]) -> &'a [u8]{
    let sz = bytes.len();
    let mut end  = sz;
    let mut hit = false;
    for i in 0..sz {
        for del_char in cut{
            if bytes[sz - 1 - i] == *del_char{
                end -= 1;
                hit = true;
                break;
            }
...
const PAGE_META_TYPE : usize = 0;
const PAGE_DATA_TYPE : usize = 256;
const PAGE_AMD_TYPE : usize = 1024;
...
impl SAS7bdat {
    ...
    fn get_decompressor(&self)  -> Option<Decompressor>{
        match dbg!(self.compression.as_str()) {
    ...
    fn get_properties(&mut self) -> Result<(), SasError>{
        ...
        match get_encoding_map().get(&usize::from(self.buf[0])) {
        ...
        match std::str::from_utf8(&self.buf[0..DATASET_LENGTH]){
            Ok(v) => self.name = dbg!(v.to_string()),
            Err(e) => {
                for bt in &self.buf[0..DATASET_LENGTH]{
                    println!("utf {}", bt);
                }
        ...
        match std::str::from_utf8(&self.buf[0..FILE_TYPE_LENGTH]){
            Ok(tp) => self.file_type = dbg!(tp.to_string()),
        ...
        let tmp_len = self.buf_rdr.read(&mut tmp_buf[0..(dbg!(self.props.hdr_len) - 288)]).expect("Read error");
        if dbg!(tmp_len) != dbg!(self.props.hdr_len - 288){
            return Err(SasError::Byte);
        }

        self.cached_page.extend_from_slice(&tmp_buf);
        if dbg!(self.cached_page.len()) != dbg!(self.props.hdr_len){
        ...
        self.props.page_len = dbg!(self.read_int(PAGE_SIZE_OFFSET + align1, PAGE_SIZE_LENGTH)?);
        ...
        match std::str::from_utf8(&self.buf[0..SAS_SERVER_TYPE_LENGTH]){
            Ok(srv) => self.server_type = dbg!(srv.to_string()),
    ...
    fn read_page_hdr(&mut self) -> Result<(), SasError>{
        ...
        dbg!(bit_off);
        dbg!(self.cached_page.len());
        self.cur_page_block_count = dbg!(self.read_int(BLOCK_COUNT_OFFSET + bit_off, BLOCK_COUNT_LENGTH)?);
        self.cur_page_sub_hdr_count = dbg!(self.read_int(SUBHEADER_COUNT_OFFSET + bit_off, SUBHEADER_COUNT_LENGTH)?);
        self.cur_page_type = dbg!(self.read_int(PAGE_TYPE_OFFSET + bit_off, PAGE_TYPE_LENGTH)?);
        Ok(())
    }
    ...
    fn get_sub_hdr_idx(&self, sig : Vec<u8>, compression: usize, ptype : usize) -> Result<usize, SasError> {
        let sub_hdr_sig_map = get_hdr_sig_map();
        match sub_hdr_sig_map.get(&sig){
            Some(val) => Ok(*val),
            None => {
    ...
    fn process_col_txt_sub_hdr(&mut self, mut off : usize, len : usize) -> Result<(), SasError> {
        ...
        match std::str::from_utf8(&self.buf[0..txt_block_sz]){
            Ok(nm) => self.col_name_strings.push(nm.to_string()),
            Err(_) => return Err(SasError::SasProperty("Could not read column name".to_string())), 
        }
    ...
    fn process_col_list_sub_hdr(&mut self, off : usize, len : usize) -> Result<(), SasError>{
        Ok(())
    }
    ...
    fn is_page_metamix_amd(&self, page_type : usize) -> bool{
        matches!(page_type, PAGE_META_TYPE | 512 | 640 | PAGE_AMD_TYPE)
    }
    fn is_page_mix_type(&self, val : usize) -> bool{
        matches!(val, 512 | 640)
    }
    fn is_page_mix_data_type(&self, val : usize) -> bool {
        matches!(val, 512 | 640 | 256)
    }
    fn check_page_type(&self, cur_page : usize) -> bool {
    ...
    fn read_int_from_buf(&self, w : usize) -> Result<usize, SasError> {
        ...
        match w {
            1 => match dbg!(i8::from_bytes(&self.buf, 0, 1, &self.byte_order)).try_into(),
            2 => dbg!(i16::from_bytes(&self.buf, 0, 2, &self.byte_order)).try_into(),
            4 => dbg!(i32::from_bytes(&self.buf, 0, 4, &self.byte_order)).try_into(),
            8 => dbg!(i64::from_bytes(&self.buf, 0, 8, &self.byte_order)).try_into(),
    ...
    fn read_next_page(&mut self) -> Result<bool, SasError>{
        ...
        let n = self.buf_rdr.read(&mut self.cached_page).expect("Could not read from bufreader!");
        if n == 0 {
            return Ok(true);
        }
        if self.cached_page.len() != self.props.page_len{
            return Err(SasError::SasProperty("Failed to read complete page from file".to_string()));
        }
        self.read_page_hdr()?;
                    ...
                    Ok(x) => {
                        match self.string_pool_r.get(&x.to_string()) {
                            ...
                            None => {
                                ...
                                self.string_pool_r.insert(x.to_string(), 5);
    ...
    fn read(&mut self, num_rows : usize) -> Result<(), SasError>{
        ...
        for j in 0..dbg!(self.props.col_cnt){
...
fn rle_decompress(res_len : usize, input : &[u8]) -> Result<Vec<u8>, SasError>{
    ...
    while !inbuf.is_empty() {
        let control_byte = dbg!(inbuf[0] & 0xF0);
